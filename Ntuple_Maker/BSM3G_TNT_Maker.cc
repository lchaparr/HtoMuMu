  ...
  //now do what ever initialization is needed
{
  ...
  _fillFSRphotoninfo       = iConfig.getParameter<bool>("fillFSRphotoninfo");
  ...
  if(_fillFSRphotoninfo)     FSRphotonselector = new FSRPhotonSelector("miniAOD", tree_, debug_, iConfig, consumesCollector());
}
...
BSM3G_TNT_Maker::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  if( _fillFSRphotoninfo)       FSRphotonselector->Fill(iEvent);
  tree_->Fill();
