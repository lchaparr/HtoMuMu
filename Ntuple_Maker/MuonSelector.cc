...
MuonSelector::MuonSelector(std::string name, TTree* tree, bool debug, const pset& iConfig, edm::ConsumesCollector&& iCC):baseTree(name,tree,debug){
  ...
  _trigObjsToken    = iCC.consumes<pat::TriggerObjectStandAloneCollection> (iConfig.getParameter<edm::InputTag>("trigObjs"));
  _trigResultsToken = iCC.consumes<edm::TriggerResults>                    (iConfig.getParameter<edm::InputTag>("triggerResults"));
...
// -------------------
  ...
  if(debug_) std::cout << "     MuonSelector: Looping over muons." << std::endl;
	...
	_trigNames.push_back("HLT_IsoMu24_");
	_trigNames.push_back("HLT_IsoTkMu24_");
 ...
 // Basic trigger (which triggers are in the list? - AWB 12.11.16)
    bool MuTriggerMatch = false;
    for (unsigned int iTrig = 0; iTrig < _trigNames.size(); iTrig++) {
       if(IsHltMatched( mu, iEvent, iSetup, trigObjsHandle, trigResultsHandle, _trigNames[iTrig], 0.1 ))
         MuTriggerMatch = true;
    }
//	cout<<"Match with trigger:"<<MuTriggerMatch<<endl;

    // fill root tree with "necessary" information:  kinematics, ID, isolation    
    Muon_isTriggerMatched.push_back(MuTriggerMatch);
...
void MuonSelector::SetBranches(){
  ...
  AddBranch(&Muon_isTriggerMatched	,"Muon_isTriggerMatched");
...
bool MuonSelector::IsHltMatched( edm::View<pat::Muon>::const_iterator muon, const edm::Event& iEvent, const edm::EventSetup& iSetup,
  ...
  for (unsigned iTrig = 0; iTrig < nTrigs; ++iTrig) {
    ...
    if ( (trigName.find(_desiredTrigName)!= string::npos) && _trigResultsHandle->accept(iTrig) ) {
      stringstream debugString;
     /* cout << "IsHltMatched: ";
      cout << "'" << _desiredTrigName << "'\n";
      cout << "  Trigger " << iTrig << ": " << trigName << "(" << trigNameStripped
	...
	if (isRightObj) {
	//  cout << "    TriggerObject:  " << tmpTrigObj.collection() << endl;
