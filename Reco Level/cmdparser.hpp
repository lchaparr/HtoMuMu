...
namespace cli {
	...
	public:
		...
		void set_optional(const std::string& name, const std::string& alternative, T defaultValue, const std::string& description = "", bool dominant = false) {
			...
			command->default_value = defaultValue;
			_commands.push_back(command);
		}
		...
		template<typename T>
		T getAlternative(const std::string& name) const {
			for (const auto& command : _commands) {
				if (command->alternative == "--"+name) {
					auto cmd = dynamic_cast<CmdArgument<T>*>(command);
		...
		template<typename T>
		std::string isNonDefault(const std::string& name) const {
		    ...
		    for (const auto& command : _commands) {
				if (command->alternative == "--"+name) {
					...
					if (cmd->value != cmd->default_value){
					    oss << name << "_"  << cmd->value;
					    return oss.str();}
					else{
					    return "";
					}
